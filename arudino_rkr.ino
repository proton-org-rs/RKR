...
void handleLine(char *s) {
  ...
  if (!strncmp(s, "MOVE1", 5)) {
    ...
    if (sscanf(s, "MOVE1 %ld", &steps) == 1) {
      ...
      if((stepper1.currentPosition()>=560 && steps < 0) || (stepper1.currentPosition()<=-560 && steps > 0) || (stepper1.currentPosition()>-560 && stepper1.currentPosition()<560) ){
        stepper1.move(steps);
      Serial.println(F("OK MOVE1"));
      }
      else {
        stepper1.stop();
        Serial.println(F("doslo do kraja"));
      }
  ...
  if (!strncmp(s, "MOVE2", 5)) {
    ...
    if (sscanf(s, "MOVE2 %ld", &steps) == 1) {
      ...
      if((stepper2.currentPosition()>=560 && steps < 0) || (stepper2.currentPosition()<=-560 && steps > 0) || (stepper2.currentPosition()>-560 && stepper2.currentPosition()<560) ){
        stepper2.move(steps);
